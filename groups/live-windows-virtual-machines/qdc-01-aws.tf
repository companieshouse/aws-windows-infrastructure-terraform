# ------------------------------------------------------------------------------
# EC2 Instance - Quest - DC Server (For DR)
# ------------------------------------------------------------------------------
# ------------------------------------------------------------------------------
...
locals {
  qdc_1_subnet_id = coalesce(data.aws_subnets.application.ids...)
}
...
module "qdc_1_ec2" {
  ...
  ami                    = var.qdc_1_ami
  instance_type          = var.qdc_1_ec2_instance_size
  key_name               = aws_key_pair.qdc_1_keypair.key_name
  monitoring             = var.monitoring
  get_password_data      = var.get_password_data

  vpc_security_group_ids = [
  #  module.qdc_1_security_group.security_group_id,
  ...
  subnet_id            = local.qdc_1_subnet_id
  iam_instance_profile = module.qdc_1_profile.aws_iam_instance_profile.name
...
data "aws_subnet" "qdc_1" {
  id = local.qdc_1_subnet_id
}
...
resource "aws_volume_attachment" "qdc_1" {
  for_each = { for v in var.ebs_volumes_qdc_1 : v.name => v }

  device_name = each.value.device_name
  volume_id   = aws_ebs_volume.qdc_1[each.key].id
  instance_id = module.qdc_1_ec2.id
