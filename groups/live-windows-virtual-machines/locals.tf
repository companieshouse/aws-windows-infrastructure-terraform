...
# Live foldlive_1 locals
  ...
  # Live smartv_2 locals
  # ------------------------------------------------------------------------------

  smartv_2_ec2_data = data.vault_generic_secret.smartv_2_ec2_data.data

  #For each log map passed, add an extra kv for the log group name
  smartv_2_cw_logs = { for log, map in var.smartv_2_cw_logs : log => merge(map, { "log_group_name" = "${var.application}-smartv-2-${log}" }) }
  ...
  doc1pg_ec2_data = data.vault_generic_secret.doc1pg_ec2_data.data

  #For each log map passed, add an extra kv for the log group name
  doc1pg_cw_logs = { for log, map in var.doc1pg_cw_logs : log => merge(map, { "log_group_name" = "${var.application}-doc1pg-${log}" }) }

  doc1pg_log_groups = compact([for log, map in local.doc1pg_cw_logs : lookup(map, "log_group_name", "")])

      # Live doc1tdg locals
  # ------------------------------------------------------------------------------

  doc1tdg_ec2_data = data.vault_generic_secret.doc1tdg_ec2_data.data

  #For each log map passed, add an extra kv for the log group name
  doc1tdg_cw_logs = { for log, map in var.doc1tdg_cw_logs : log => merge(map, { "log_group_name" = "${var.application}-doc1tdg-${log}" }) }
...
# ------------------------------------------------------------------------------
  #  Live smartv_2 Security Group Variables
  # ------------------------------------------------------------------------------
...
# ------------------------------------------------------------------------------
  #  Live doc1tdg Security Group Variables
  # ------------------------------------------------------------------------------
